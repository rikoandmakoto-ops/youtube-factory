...
最終更新: 2026-09-09（09-08 深夜 PDCA / 全進捗レポートの指摘6件を修正）

> **2026-09-09 の変更まとめ** — 09-08 深夜に見つかった「設定したのに効いていない」を全部潰した回
>
> | 内容 | 状態 |
> |---|---|
> | 🚨 **語彙照合の素の部分一致を根本修正** | ✅ 新設 `pipeline/jp_wordmatch.py`。文字種の切れ目を語境界として使い、①1文字の漢字は漢字に挟まれていたら不採用（月**額**／言**葉**／注**目**／**影**響／意**味**／能**力**／本**音**）②カタカナ語はカタカナの連なり全体と一致し、直後に漢字が来たら不採用（**ロケット**団／アニメ版）③ラテン文字は連なり全体と一致。ひらがな側は活用があるので境界を見ない（「見えない」は当たったまま）。`_match_textbook` / `_leaked_matched` の両方が経由する＝**全13ch同じ規則**。形態素解析器は入れていない（Python 3.9・常駐・依存を増やさない） |
> | ch単位の `thumbnail_card:false` を撤去 | ✅ `pokemon-lab` / `scp-lab` / `yokai-watch` の潰しフラグを外した。フラグ自体は残すが**誤爆対策をそこでやらない**旨を各JSONに明記。実測（09-01以降の公開本）で描画率は daily-science 90→77 / scp-lab 45→39 / yokai 2→0。減った分は「ロケット団→ロケット」型の偽陽性 |
> | 1文字漢字で拾えなくなる2字熟語 | ✅ 規則を緩めず**語彙側に明示**して取り戻した（寝落ち／湿度／発熱／秒間／脳内／毛穴／静電気／星空 ほか）。緩めると月額・言葉・能力が全部戻る |
> | 🚨 **`require_answer_marker` が1行も読まれていなかった** | ✅ `hard_constraints` に **`require_any_of`**（「必ず含む」制約）を実装。検査は語境界つき（「理不尽」の「理」で『理由』を満たさない）。違反は LLM 再生成2回 →最後は **語尾に足すだけ**の機械修復（語順・助詞は動かさない）。対象6ch（daily-science / scp-lab / pokemon-lab / yokai-watch / 2ch-matome / company-facts）を移行済み |
> | 実効値 | ✅ 09-07〜08 の公開33本で検証: **素通り 18/33（55%）→ ゲート後 33/33（100%）** |
> | 🆕 `_is_broken_japanese` が `original` を見ていなかった | ✅ **既存バグ**。「子ど**もを**」が助詞連結パターンに、「**になっ**た」が助詞＋述語パターンに当たるため、**正常なタイトルはどんな機械修復も一律に捨てられていた**（数字除去の修復も同様）。原文に無い並びが新しく生まれたときだけ壊れたと見なすよう修正。「はで」「でずつ」型の検出力は維持 |
> | 🚨 **2ch-matome のテーマを結論型へ差し替え** | ✅ 09-08 の指揮者が書いた `theme_queue` は**存在しないキー**だった。本物の `theme_seeds` 28件を全て結論型に入れ替え（参加型 登録/千 0.06 vs 結論型 0.32＝5.3倍）。あわせて**実際に消費される `autopilot.theme_queue`**（参加型1件・重複1件を除去し補充）と `data/channels/2ch-matome/theme_queue.json`（12件全入れ替え）も揃えた＝**3箇所すべて**。『なんか〜んやが』型（登録/1k 0.60・ch平均の3.5倍）はフックとして残し、後半に答えの提示を足して両立 |
> | 答え提示語の割り当て | ✅ 1語に寄せると `cross_channel_gate` の同日上限に毎日当たるので、2ch は 理由／わけ／実は を回している |
> | 🆕 **daily-science の theme_blacklist 棚卸し** | ✅ **42語 → 16語**。①一度も何も止めていなかった23語を削除（天体／銀河／免疫力／悲しい曲／42度／毎晩3時／3割増し／指パキ ほか）②領域ごと封じていた語を実際の重複題材まで縮小（宇宙・星→**星空/星座** ／ 録音・骨伝導→**自分の声** ／ 睡眠の深さ→**寝言** ／ 炭酸→**炭酸を飲むと** ／ ブルーライト→`re:(深夜\|夜\|暗い部屋).{0,8}スマホ` ／ 本を読むと→`re:本.{0,8}眠`）。**非回帰をテストで担保**（旧42語が止めていた過去タイトル27件は新16語でも全件ブロック） |
> | 🚨 **`video_publish.db` の予約行が published にならない** | ✅ 新設 `publish_log.reconcile_scheduled()`。予約公開は `status='scheduled'` / `published_at=NULL` で書かれるが、**予定時刻が来たことを誰も書き戻していなかった**。`published_at` で集計する経路（pdca_report / 公開本数 / api_improvement）から丸ごと落ちていた。**滞留136行を published に更新済み**（バックアップ `data/video_publish.db.bak_20260909`）。backend 起動時と PDCA の集計前に自動実行する |
> | 判定根拠 | 予約時刻ベース（YouTube API は叩かない）。予約は API 受理時点で確定し `video_id` も採番済み。API 照合は quota を使い、OAuth 失効chで丸ごと失敗して復旧が遅れる |
> | 🆕 **`video_metrics` の集計タイミング** | ✅ 両方やった。①`com.youtube-factory.pdca` の起動を **23:00 → 22:30** に前倒し（sync は約20分かかるので 23:00 には全ch揃う。plist 更新＋reload 済み）②新設 `pipeline/analytics/freshness.py` と `backend/check_metrics_freshness.py` で**欠測検知**。PDCA レポートに「### 計測データの充足」節が毎日出る。**0行を「実績0」や「取得障害」と読ませない** |
> | 欠測検知の実測 | 09-08 は 9/12ch。欠測は `akashic-librarian` / `clip-animal` / `fake-paper` ＝ **OAuth 失効ch と完全一致**（タイミング問題ではなく本物の欠測だった） |
> | テスト | ✅ `backend/tests/test_fixes_20260909.py`（48件）。全 **508件中、失敗は既知の3件のみ**（pytest 未導入の `test_cta_enforcer` / clip-animal・clip-kaneko のハッシュタグ数） |
> | A/B テスト | ⚠️ `cta_position`（〜09-11）・4行目ルール（〜09-13）・尺の対照群（〜09-12）には**一切触れていない** |
>
> ⚠️ **backend の再起動が要る。** 稼働中プロセスはコードもチャンネルJSONもメモリに抱えているので、
> `launchctl kickstart -k gui/$(id -u)/com.youtube-factory.backend` を通すまで上の修正は効かない。
>
> ⚠️ **持ち越し（コード側では直せないもの）**: `ANTHROPIC_API_KEY` の貼り直し（`backend/.env` 18行目がコメントアウト）/
> OAuth 4ch の再認可（`clip-animal` / `socio-rx` / `fake-paper` / `akashic-librarian`）/ git push の認証 /
> 画像ブリッジのワーカー / `clip-lab` の登録転換。

> **2026-09-08 深夜の点検結果は `MEMORY_UPDATE_20260908_night.md`。**要点だけ:
>
> | 項目 | 実測 |
> |---|---|
> | 🚨🚨🚨 **本日の指揮者の変更が2件とも「読まれないキー」に入っていた** | ①`title_rules.require_answer_marker` は **backend が1行も読まない**（`grep` 0件。`title_constraints.py` の冒頭が「09-03 の検証で確定」と自ら明記）。読まれるのは `title_rules.hard_constraints` だけで、そのスキーマは**禁止系の制約しか無く「必ず含む」を表現できない** ②2ch-matome のテーマ入れ替えは `theme_queue` に書いたが、**チャンネルJSONにそんなキーは存在しない**。本物は `theme_seeds` で、**今朝のバックアップとタイトル列が完全一致＝1件も変わっていない** |
> | 実効値 | 09-08 公開18本の**答え提示語 適合は 11/18（61%）。pokemon-lab は 0/3。**設定変更後の13本でも 8/13 |
> | 🆕 **`theme_queue.json` は揮発キャッシュ** | `data/channels/<ch>/theme_queue.json` は **30分ごとに `theme_seeds` から自動再補充**（`target_size=10`）。**ここを直しても30分で戻る。恒久的に変えるなら必ず `theme_seeds`** |
> | 2ch-matome の実効 | `theme_seeds` 28件中 **参加型18 / 答え提示1**。`theme_queue.json` 12件中 **参加型7 / 答え提示0**。**本日最大の変更は1本も動画に出ていない** |
> | 本日の公開 | **25本**（09-07 の27本から -2）。減少分は**すべて OAuth 失効ch**（akashic 2→0 / fake-paper 1→0）。**稼働9chは全枠成功** |
> | 枠移設の一過性 | ✅ **予告どおり解消**。pokemon-lab 4→3 / yokai-watch 2→3。移設3枠（company-facts 19:00 / pokemon-lab 17:00 / yokai-watch 17:00）はすべて反映済み。`restart_and_trigger_20260908.command` は**未実行だが不要だった** |
> | 🆕 🚨 **`ANTHROPIC_API_KEY`** | `backend/.env` 18行目に **`#` でコメントアウトされた状態で存在**する（09-07 の「無い」より正確）。値も40字程度と短く**貼り直しが要る**。`.env` の更新は 08-19 で以降ノータッチ。本日も `viral エンジンが失敗: ANTHROPIC_API_KEY 未設定` が9回。`APIキー調査結果.md` の「キーあり」は**この行の読み違い** |
> | サムネ403 | **22/25本（88%）**。免れたのは **daily-science 3本のみ**。🆕 **09-07 の company-facts 07:34 の不整合は再現せず**（本日3枠とも403）＝「電話認証済みchだけ通る」の理解は疑わなくてよい。akashic 失効でサムネ付き供給元は **daily-science 1chだけ**になった |
> | OAuth | `clip-animal` / `socio-rx` / `fake-paper` / `akashic-librarian` の**4chが失効のまま2日目**。他9chは **09-08 23:00 にリフレッシュ成功** |
> | 🆕 画像ブリッジ | `pending` **210 → 57**、🆕 **`failed` 0 → 235**。理由は全件「対象動画が既に投稿済みで使い道が無い」＝**事故ではなく棚卸し**。`delivered` は**4日連続 0**。残るはワーカーを人が回すことだけ |
> | ✅ `yokai-watch` テーマ枯渇 | **解消**。8（残2.7日）→ **27（残9.0日）**。本日の指揮者が補充済み |
> | 🆕 `video_metrics` の取得時刻 | **23:01〜23:19 に ch名のアルファベット順で埋まる**（実行中に scp-lab 18→50行、yokai 0→48行を観測）。**23:20 より前に集計すると後半chが「欠測」に見える。**09-05/06 の「yokai 2日連続0行」はこれの可能性 |
> | 09-08 スナップショット | moviepy6ch **290本 / 再生215,518 / 登録105 / 登録/千 0.487**。★ 朝の分析（09-07・286本/213,403/101/0.4733）を**独立再計算して完全一致** |
> | ★ 答え提示語 **5回目の再現** | **1.83倍**（0.59 vs 0.32・n=202）。維持率は**逆行**（44.3% vs 49.6%）。08-29→09-03→09-04→09-08朝→09-08夜 で5回連続同方向。**維持率は捨ててよい、が確定** |
> | ★ 2ch 参加型 vs 結論型 | **0.06 vs 0.32（5.3倍）で再現。**判定は正しい。⚠️ ただし施策が実効配置されていないので **09-15 の反証期限は「施策の検証」にならない** |
> | 🚨 `clip-lab` | **再生42,321で全ch1位・登録1・登録/千 0.02 で最下位**（2ch-matome の10分の1）。「再生は出るが登録に変わらない」の最悪例。09-07 に主題宣言済みで**2日連続未着手** |
> | 🆕★ 新規プロジェクト | **`client-ops-platform`**（クライアント対応自動化・仮称）が 09-06 開始・**本日16コミットで全プロジェクト中最多**。Chatwork / 公式LINE / LINE WORKS 連携、Vercel+Neon 移行、Vercel Cron。**ワーカー13本は Cowork 定期タスクとして1つも登録されていない** |
> | 🚨 git push | `daily-merge-all-projects` が youtube-factory 5 / aiseki 2 / ai-english-coach 1 コミットしたが **push は全滅**（認証不可2件・ai-english-coach は**リモート未設定**）。✅ LibreOffice ロックファイルは `.gitignore` 済みで解消 |
>
> **明日の優先順**: ①`hard_constraints` に `require_any_of` を実装 ②2ch-matome を **`theme_seeds`** に対して入れ替え直し ③`ANTHROPIC_API_KEY` 貼り直し＋再起動 ④OAuth 再認可（本番公開→再認可の順） ⑤git push 認証 ⑥client-ops-platform のワーカー登録 ⑦画像ワーカー ⑧clip-lab の登録転換
...
## 12. 2026-09-09 午前の記録 — 設定が消える経路と、scp-lab の二重枠

詳細は `MEMORY_UPDATE_20260909.md`。要点だけ。

### 12-1. 🚨 稼働中バックエンドが指揮者の設定変更を上書きする（本日の最重要）

10:38 に反映した `data/channels/yokai-watch.json` が **11:15:13 にバックエンドに
書き戻され、`banned_words` / `forbid_patterns` / `title_style` / `pdca_log` の
09-09 差分が丸ごと消えていた。** 検証で気づいて再適用した。

`theme_queue` の再生成のたびにチャンネル JSON 全体が書き戻され、
書き戻す側が持っているのは**プロセス起動時に読んだツリー**なので、
その後のファイル編集は差分ごと消える。

> **設定を変えたら必ず読み直して残っているかを確認すること。**
> 書き込み成功のログは何の保証にもならない。
> 検証は `scripts/orch_verify_20260909.py`（symlink・枠の重複・ゲートの実効性・誤爆）。

### 12-2. 🚨 `scp-lab` の投稿枠が二重登録で1日6本作っていた

`times` が `[(17,0),(13,0),(19,0),(17,0),(13,0),(19,0)]`。
`sch.add_job(id=_job_id(channel_id, idx))` の **ID が枠の添字**なので、
同一時刻でも添字が違えば別ジョブとして生き、2回発火する。3枠へ修正した。

> ⚠️ **再起動しないと idx=3,4,5 の古いジョブが残る**（`replace_existing` は
> idx 0〜2 にしか効かない）。再起動まで scp-lab は1日6本作り続ける。

### 12-3. タイトルの実測（n=196・公開09-05以前・views>=200・登録/千再生）

| 発見 | 該当 | 非該当 |
|---|---:|---:|
| 「秘密」 | **0.120** (n=18) | 0.527 (n=178) |
| 「99%が知らない」型 | **0.194** (n=11) | 0.510 (n=185) |
| 2ch「ワイ/質問ある」 | **0.000** (n=11・10,781再生) | 0.282 (n=27) |
| scp「— …【ラベル】」型 | **0.000** (n=6) | 0.960 (n=30) |
| scp `SCP-数字ID` あり | **0.888** (n=27) | 0.449 (n=9) |

いずれも `title_rules.hard_constraints` へ反映済み（`SCP-数字ID` の必須化を除く）。
「秘密」は出現する4ch全てで同方向、かつ公開が古い側＝時間バイアスは結論と逆向き。

答え提示語は**語ごとに倍違う**: 実は 0.767 > 実態 0.681 > なぜ 0.568 >
正体 0.510 > 本当 0.462 > 理由 0.368。最上位の「実は」は語尾に置けないので
`repair_with` には入れられず、`title_style` の自然文で指示している（＝無視されうる）。

### 12-4. コード変更が要る2件（本日は未実施）

1. `title_constraints.py` の `_REASON_WORDS` から **「秘密」を外す**。
   「なぜ」始まりの言い換え時に「秘密」を理由語と誤認して補正を止める。実測では最悪の語。
2. `hard_constraints` に **`require_pattern` を追加**する。
   `require_any_of` は語彙照合のみで、`SCP-\d+` のような必須パターンを表現できない。

### 12-5. サンドボックスから外に出られない（Phase 1 / Phase 4）

`googleapis.com` はタイムアウト、ホストの `localhost:8000` は 403。
**実績取得と制作指示は指揮者側では実行できない**（`trigger_20260909.command` に集約）。
ただし **Phase 4 は APScheduler が枠ごとに自動発火する**ので、
手動 trigger は計画に**上乗せ**になる。叩かないのが正しい。

`reports/*.xlsx` の `recalc.py`（LibreOffice）は初回のみ成功し以後ハング。
代替の `scripts/orch_cache_values.py` で数式を残したままキャッシュ値を注入した。
**中断した LibreOffice は空の `<v/>` を残す**ので、要素の有無ではなく
中身が空でないかで判定すること（一度これで空のまま出荷しかけた）。
