"""Full pipeline runner for the 'why-can't-we-run-in-dreams' video.

Loads channel + scenario + .env, then drives generate_all() so the new
HTML+Playwright thumbnail path (which needs channel_dict) is exercised.
"""
import os
import sys
import json
...
SCENARIO_PATH = ROOT / "data" / "scenarios" / CHANNEL_ID / "なぜ夢の中では走れないのか.json"
...
# Resolve background from channel default
...
result = generate_all(
    ...
    channel_dict=ch.to_dict(),             # ← enables HTML+Playwright thumbnail
)
