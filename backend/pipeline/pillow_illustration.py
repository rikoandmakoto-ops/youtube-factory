...
# 語彙照合は「語として当たったか」を見る（素の部分一致だと無関係な語の内側に
# 刺さる。2026-09-09 の根本修正 → pipeline/jp_wordmatch.py）。
try:  # 単体でも import できるように（スクリプトから直接叩く経路がある）
...
#
# 【2026-09-09】照合は `jp_wordmatch` の語境界判定に変わった（→ `_hits`）。
# 1文字の漢字は「漢字に挟まれていたら複合語の内側」として落ちるので、
# **拾いたい2字熟語は明示的に並べる**こと。規則を緩めて拾おうとすると
# 「月額」の月・「言葉」の葉・「能力」の力が全部戻ってくる。
# 例: 「寝落ち」「湿度」「発熱」は 寝/湿/熱 の単字では当たらないので直接書く。
_TEXTBOOK_KEYWORDS = [
    (("光合成",), ("leaf", "光合成")),
    # --- 体・生理系(daily-science は身近な体の不思議テーマが多いので優先的に先頭付近へ) ---
    (("骨伝導", "骨", "骨格", "骨折"), ("bone", "骨")),
    (("汗", "発汗", "手汗", "寝汗"), ("droplet", "汗")),
    (("呼吸", "肺", "息", "酸欠", "深呼吸"), ("lungs", "呼吸")),
    (("筋肉", "筋", "こむら", "つる", "筋トレ"), ("muscle", "筋肉")),
    (("皮膚", "肌", "鳥肌", "かゆ", "湿疹", "毛穴"), ("skin", "皮膚")),
    (("胃", "胃酸", "空腹", "消化", "胃袋"), ("stomach", "胃")),
    (("腸", "便", "腸内", "お腹", "腹痛"), ("intestine", "腸")),
    (("耳", "聴覚", "鼓膜", "耳鳴り"), ("ear", "耳")),
    (("鼻", "嗅覚", "くしゃみ", "匂い", "におい", "鼻水"), ("nose", "鼻")),
    (("舌", "味覚", "味", "唾液"), ("tongue", "味覚")),
    (("歯", "虫歯", "咀嚼", "噛", "歯磨"), ("tooth", "歯")),
    (("髪", "毛", "抜け毛", "白髪", "体毛"), ("hair", "髪")),
    (("手", "指", "爪", "握"), ("hand", "手")),
    (("睡眠", "眠", "寝", "夢", "あくび", "寝落ち", "寝不足", "就寝", "昼寝"),
     ("moon", "睡眠")),
    (("血液", "血流", "血管", "貧血"), ("heart", "血液")),
    (("太陽", "日光", "恒星", "サン"), ("sun", "太陽")),
    (("月", "ムーン", "満月", "三日月"), ("moon", "月")),
    (("惑星", "地球", "宇宙", "星", "天体", "星空", "星座"), ("planet", "惑星")),
    (("ロケット", "宇宙船", "打ち上げ"), ("rocket", "ロケット")),
    (("酸素", "気体", "空気", "大気", "二酸化炭素"), ("cloud", "気体")),
    (("水", "海", "液体", "水分", "湿", "湿度", "湿気"), ("water", "水")),
    (("炎", "火", "燃焼", "熱", "高温", "発熱", "加熱", "熱中症"), ("fire", "熱")),
    (("氷", "雪", "冷", "低温", "凍", "冷蔵", "冷却", "冷凍"), ("snow", "氷")),
    (("雷", "電気", "電流", "電子", "放電", "感電", "静電気"), ("bolt", "電気")),
    (("磁", "磁石", "磁場", "磁力"), ("magnet", "磁力")),
    (("原子", "分子", "元素", "化学", "物質"), ("atom", "原子")),
    (("DNA", "遺伝", "ゲノム", "染色体"), ("dna", "遺伝子")),
    (("脳", "神経", "記憶", "思考", "脳内"), ("brain", "脳")),
    (("心臓", "血", "血液", "鼓動", "循環"), ("heart", "心臓")),
    (("目", "視覚", "視力", "網膜", "光", "見え", "眼球"), ("eye", "視覚")),
    (("細胞", "菌", "ウイルス", "微生物", "バクテリア", "感染", "細菌", "殺菌"),
     ("microbe", "細胞")),
    (("植物", "葉", "緑", "森", "木", "樹木"), ("leaf", "植物")),
    (("歯車", "機械", "仕組み", "装置", "エンジン", "構造"), ("gear", "仕組み")),
    (("時間", "時計", "速度", "秒", "周期", "秒間", "数秒", "瞬間"), ("clock", "時間")),
    (("音", "振動", "波", "周波", "音波", "騒音", "音量"), ("wave", "波")),
    (("山", "地震", "火山", "地層", "岩", "岩石"), ("mountain", "地形")),
...
def _hits(topic, keys):
    """語彙 `keys` のいずれかが `topic` に**語として**現れるか。

    2026-09-09: 素の部分一致（`k in topic`）をやめた。理科系の一般語を素の
    部分一致で当てると、題材が虚構・固有名詞のチャンネルで無関係な語の内側に
    刺さる（「ロケット団」→ロケット / 「言葉」→葉 / 「月額」→月 / 「影響」→影）。
    以前はチャンネル単位で `thumbnail_card: false` にして潰していたが、
    当たっているチャンネルまで機能を失ううえ、チャンネルが増えるたびに
    同じ事故が出る。照合規則を `jp_wordmatch` に一本化した（全13ch共通）。
    """
    return _jw.any_word(topic or "", keys)
...
def _leaked_matched(topic) -> bool:
    ...
    2026-09-09: こちらも `_hits` 経由にした。1文字の漢字（目・像・影・姿・箱・鏡）
    が並んでいるので、素の部分一致だと「影響」→人影・「駄目」→視認体 のように
    ほぼ全てのタイトルが何かに当たってしまい、判定として機能していなかった。
    """
    return any(_hits(topic, keys) for keys, _ in _LEAKED_KEYWORDS)
...
def has_confident_render(topic, *, card_style="textbook", use_keyword_icons=True) -> bool:
    ...
    2026-09-09: 語彙照合を `jp_wordmatch` の語境界判定に置き換えた。従来は素の
    部分一致だったため「当たっている」と誤認する側に倒れており、pokemon-lab /
    yokai-watch / scp-lab はチャンネル単位でカードごと切って回避していた。
    判定が直ったのでチャンネル側のフラグは外してある（→ 全13ch同じ規則）。
    """
    t = (topic or "").strip()
    if not t:
        return False
