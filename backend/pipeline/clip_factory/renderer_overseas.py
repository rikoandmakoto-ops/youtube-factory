...
def compute_layout(
    ...
    avail_h = (cta_y - layout.gap - sub_band - layout.gap - accent_h
               - layout.gap - hook_block - top_min)
    if video_h > avail_h and avail_h > 0:
        scale = avail_h / video_h
    ...
    block = (hook_block + layout.gap + accent_h + layout.gap + video_h
             + layout.gap + sub_band)
    hook_y = max(top_min, (cta_y - layout.gap * 2 - block) // 2)
    ...
    return ComputedOverseasLayout(
        ...
        cta_y=cta_y,
        video_w=video_w,
        video_x=(layout.width - video_w) // 2,
    )
...
def render_clip(
    *,
    ...
    computed = compute_layout(layout, hook=hook, source_size=probe_size(source_path),
                              reserve_subtitles=bool(subtitle_lines))
    ...
    filters = [
        ...
        f"[0:v]crop=iw:ih*{keep:.4f}:0:0,"
        f"scale={computed.video_w}:{computed.video_h},setsar=1[src]",
        f"[bg][src]overlay=x={computed.video_x}:y={computed.video_y}:shortest=1[v0]",
...
def render_thumbnail(
    *,
    ...
    `reserve_subtitles` は render_clip と同じ値を渡すこと。揃えないと本編と
    サムネで映像の位置・大きさがズレる。
    """
    try:
        ...
        computed = compute_layout(layout, hook=hook,
                                  source_size=probe_size(source_path),
                                  reserve_subtitles=reserve_subtitles)
