...
# 動詞・形容詞の語幹に付く送り仮名。漢字列の直後にこれが来る場合、その漢字列は
# 名詞ではなく用言の語幹なので、キーワードとして切り出してはいけない。
#
# 【2026-08-31 修正】当初この集合に の が に と も で て な か という最頻出の助詞を
# 含めてしまい、「宇宙の謎」「妖怪の伝承」のような正常な名詞まで捨てていた。
# 実データ4000行での計測では 1行あたりキーワードが 7.21→2.59（-64%）に落ち、
# キーワードがゼロになる行が 1%→14% に増えていた。助詞は全て除外する。
# なお と／て は助詞と送り仮名が衝突するため、送り仮名側を諦めて
# _KEYWORD_STOPWORDS で個別に落とす。
_OKURIGANA = "きくけこぎぐげごしすせそじずぜぞちつぢづびぶべぼまみむめらりるれろわいえおっ"

# 単独で切り出しても意味をなさない機能語的な漢字列、および
# 送り仮名が助詞と衝突して弾けなかった用言の語幹。
_KEYWORD_STOPWORDS = {
    # 機能語
    "何秒", "何回", "何人", "何個", "何度", "自分", "本当", "普通", "最初", "最後",
    "今日", "明日", "昨日", "全部", "一番", "以上", "以下", "場合", "結果", "理由",
    # 「と」「て」が助詞と衝突して落とせない用言の語幹（実際に本番へ混入した例を含む）
    "見落", "見当", "見立", "取立", "申出", "言出", "思出", "引出", "呼出",
    "何秒数", "個出", "寝落",
}
...
def _extract_topic_keywords(text: str) -> set:
    ...
    【2026-08-31 修正】漢字列を素朴に `[一-鿿]{2,}` で拾っていたため、送り仮名の
    手前で語が切れて非単語が生成されていた。この結果が最終行のループトリガーに
    そのまま埋め込まれ、「…待って、寝落に戻って」「…待って、見落に戻って」
    「…待って、個出に戻って」「…待って、何秒数に戻って」のような壊れた締めが
    08-24〜30 のほぼ全チャンネルで公開されていた（最終行＝登録CTAの直前）。

    対策として、漢字列の直後が送り仮名の場合は用言の語幹とみなして除外し、
    確実に名詞であるカタカナ語・型番（SCP-173 等）を優先する。
    """
    ...
    for m in re.finditer(r"[一-鿿]{2,}", text):
        ...
        # 直後が送り仮名 → 用言の語幹（寝落ち/見落とす/読み取る…）なので捨てる
        if nxt and nxt in _OKURIGANA:
            continue
        if word in _KEYWORD_STOPWORDS:
            continue
        kanji.add(word)

    # 名詞として確実なカタカナ語・型番を優先し、無ければ漢字語にフォールバックする。
    # （以前はコメントだけで、実装は単純な和集合になっていた）
    reliable = katakana | coded
    return reliable if reliable else kanji
