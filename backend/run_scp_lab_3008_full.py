#!/usr/bin/env python3
"""scp-lab チャンネルで SCP-3008「無限に広がるIKEA」長尺動画を1本フル生成 → YouTube アップロード

- テーマ: SCP-3008「無限に広がるIKEA」
...
THEME = {
    "title": "SCP-3008「無限に広がるIKEA」",
    "angle": (
        "閉店後の家具店に閉じ込められた男の記録。どこまで歩いても終わらない売り場、"
        "昼は普通の客に見えるのに夜になると牙をむく『スタッフ』、"
        "生存者たちが築いた砦。脱出は可能なのか——財団が観測した無限空間の恐怖を"
        "発見経緯・目撃証言・収容の困難さの順に、物語として引き込む構成で解説"
    ),
}
...
def main():
    ...
    prefix = f"scp3008_{int(time.time())}"
